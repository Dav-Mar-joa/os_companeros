...
app.use(session({
    secret: 'my-application-oscompaneros', // Remplacez par votre clé secrète
    ...
    store: MongoStore.create({
        mongoUrl: process.env.MONGODB_URI || `mongodb://${process.env.MONGODB_HOST}:${process.env.MONGODB_PORT}/${process.env.MONGODB_DBNAME}`,
        collectionName: process.env.MONGODB_COLLECTION || 'sessions' // Nom de la collection dans MongoDB
    }),
    cookie: {
        maxAge: 1000 * 60 * 60 * 24, // Durée de validité du cookie (1 jour)
...
//             return next(err);
//         }
//         console.log('Session attachée:', req.session); // Ajout du log
//         next();
//     });
// });
...
io.use((socket, next) => {
    ...
    session({
        ...
        // Exemple de modification et sauvegarde de la session
        if (req.session) {
            req.session.lastSocketConnection = new Date();
            console.log("tttttttttttttt",req.session.username)
...
io.on('connection', (socket) => {
    console.log("Utilisateur connecté à Socket.IO");
    const req = socket.request;
    const username = socket.request.session.user ? socket.request.session.user.username : 'Invité';
    console.log("rrrrrrrrrrrrrrrrrrrrrrr",username)
